...
{
	//沒分割必要
	if ((point_right_up(space).y == split_point.y) && bound_side == 0)
		return;
	if ((point_left_down(space).y == split_point.y) && bound_side == 1)
		return;
	if (!space->is_space())
		return;
...
void Plane::updateStitch(vector<Tile*> neighbors, Tile* tile)
{
	...
	{
		if ((LD(tile).x - RU(neighbors[i]).x == 1) && y_in_range(tile, RU(neighbors[i])))	//nei 在 tile 左邊
			RS(neighbors[i]) = tile;
		if ((LD(neighbors[i]).y - RU(tile).y == 1) && x_in_range(tile, LD(neighbors[i])))	//nei 在 tile 上面
			DS(neighbors[i]) = tile;
		if ((LD(neighbors[i]).x - RU(tile).x == 1) && y_in_range(tile, LD(neighbors[i])))	//nei 在 tile 右邊
			LS(neighbors[i]) = tile;
